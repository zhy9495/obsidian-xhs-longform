...
如果你习惯在 Obsidian 写作，但不想再把内容复制到设计软件里逐页排版，这个插件可以帮你完成封面、字体、分页和导出。左边调整设置，右边实时查看成品；确认后点击一次导出。插件会自动识别笔记中嵌入的 MP4、MOV 和 GIF：macOS 将含动态素材的页面制成实况照片，并把整套页面加入系统“照片”的独立相簿。
...
3. 在左侧调整设置，在右侧检查实时预览，选择保存位置，然后点击 **导出**。
...
也可以在导出面板底部把保存位置切换成 **电脑文件夹**，通过系统文件夹选择器保存到“下载”等任意位置。插件会在所选位置建立笔记同名子文件夹；导出完成后会显示完整路径，并提供 **打开文件夹** 按钮。
...
选择 **当前 Obsidian 仓库** 时，可以使用 `{{title}}` 配置仓库内目录；选择 **电脑文件夹** 时，可以通过系统窗口挑选任意文件夹。每次导出结束后，面板和通知都会显示图片的完整保存路径。

### 6. 动态导出

直接在笔记中使用 Obsidian 嵌入语法添加动态素材：
...
不需要在导出面板重复选择文件，也不需要选择“PNG”还是“实况照片”。插件会在分页后自动判断：

- **macOS：** 如果整篇笔记都是静态内容，仍按普通方式导出 PNG。只要有一页包含 MP4、MOV 或 GIF，插件就会在系统“照片”中新建一个以笔记名和时间命名的相簿，并按页码加入整套页面：动态页是 Apple Live Photo，静态页是普通照片。
- **原件备份：** 用户选择的仓库目录或电脑文件夹仍会保留全部原件，包括静态页 PNG 和实况页的 JPG/MOV 配对文件。发布含实况的图文时，直接从“照片”相簿选取；文件夹中的内容用于备份和后续处理。
- **Windows / Linux：** 所有页面导出静态 PNG；动态页面另外保存对应的 MP4、MOV 或 GIF 原件，可交给剪映等工具继续处理。
- 一张 Live Photo 只能对应一段动态素材，因此插件会自动保证每个动态页面最多放置一个视频或 GIF。
- 实况播放时，整张卡片的文字、底色、页脚和其他内容保持不动，只有素材原来所在的区域播放。

macOS 转换完全在本机完成。转换器已经内置，兼容 Apple Silicon 和 Intel Mac，不需要安装 Xcode、Swift 或 FFmpeg。导出面板会提前说明本次内容是否会进入“照片”；导出过程中会显示加入相簿的进度，完成通知会同时给出相簿名和原件路径。第一次使用时，macOS 可能会询问是否允许 Obsidian 控制“照片”。
...
- 支持保存到仓库内目录或电脑上的任意文件夹，并在导出后显示和打开实际保存位置。
- 自动识别笔记中的 MP4、MOV 与 GIF；macOS 将含动态素材的整套页面加入“照片”独立相簿，Windows 与 Linux 导出 PNG 和动态原件。
...
动态导出只读取当前笔记明确嵌入的 MP4、MOV 和 GIF，并写入用户选择的导出目录。macOS 实况照片功能会在临时目录运行插件内置的开源转换器；当一篇笔记包含动态页时，插件会通过固定的系统脚本新建“照片”相簿，并将整套静态照片和实况照片按页加入。插件不会运行用户提供的命令，也不会上传动态素材。

## 兼容性

- Obsidian 1.6.6 或更新版本
- macOS、Windows、Linux 桌面版 Obsidian
- 直接导出 Apple Live Photo：仅支持 macOS 11 或更新版本
- Windows 与 Linux：导出静态 PNG 和笔记中的动态素材原件
